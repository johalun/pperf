...
use std::sync::Arc;
use net2::{UdpBuilder,TcpBuilder};

const SO_REUSEPORT_LB: i32 = 0x00010000;
...
fn main() {
    let matches = clap::App::new("pperf")
        ...
        .arg(clap::Arg::with_name("server")
             ...
             .takes_value(false)
             .help("Run as server"))
        .arg(clap::Arg::with_name("server ip")
             .short("c")
             .long("client")
             .required(false)
             .takes_value(true)
        ...
        .arg(clap::Arg::with_name("packets")
             .short("n")
             .long("packets")
             .required(false)
             .takes_value(true)
        ...
        .arg(clap::Arg::with_name("background")
             .short("b")
             .long("background")
             .takes_value(false)
    ...
    let port: usize = matches.value_of("port").unwrap().parse::<usize>().unwrap_or_else(|e| {
        error!("Port not valid number: {}", e);
        std::process::exit(1);
    });

    let ncpus: usize = matches.value_of("threads").unwrap_or(&num_cpus::get().to_string()).parse::<usize>().unwrap_or_else(|e| {
    ...
    if background {
        info!("Daemonizing...");
        let stdout = std::fs::File::create("/tmp/pperf.out").expect("Error creating /tmp/pperf.out");
        let stderr = std::fs::File::create("/tmp/pperf.err").expect("Error creating /tmp/pperf.err");
        ...
        for i in 0..ncpus {
            ...
            let thread = std::thread::spawn(move || {
                ...
                if protocol == Protocol::UDP {
                    ...
                    set_sockopt(builder.as_raw_fd(), SO_REUSEPORT_LB).expect("set_sockopt");
                                ...
                                Ok(mut stream) => {
                                    loop {
                                        ...
                                        if let Ok(_) = stream.read_exact(&mut buf) {
                                            let msg = std::str::from_utf8(&buf).expect("Error parsing message");
    ...
    if client && !server {
        ...
        let server_ip: &str = matches.value_of("server ip").expect("Error getting server IP from args.");

        let mut addresses = vec!();
        if server_ip.contains('-') {
            let range_vec: Vec<&str> = server_ip.split('-').collect();
            let addr_vec: Vec<&str> = range_vec[0].split('.').collect();
            let base: u8 = addr_vec[3].parse::<u8>().expect("Error parsing server ip range start to u8");
            let end: u8 = range_vec[1].parse::<u8>().expect("Error parsing server ip range end to u8");;
            assert!(base < end);
            for i in base..end+1 {
                addresses.push(format!("{}.{}.{}.{}:{}", addr_vec[0], addr_vec[1], addr_vec[2], i, port));
            }
        } else {
                            ...
                            .connect(addr.clone()) {
                                Ok(mut stream) => {
                                    while running.load(Ordering::SeqCst) && sent < packets_send_count {
                                        ...
                                        let _ = stream.write_all("PING".as_bytes()).expect(
                                            "Client stream write",
                                        );
                                        let _ = stream.read_exact(&mut buf).expect("Client stream read");
                                        debug!("{:02}: Got reply: {:?}", tid, std::str::from_utf8(&buf));
                                    }
                                    debug!("{:02}: Sending FINI", tid);
                                    let _ = stream.write_all("FINI".as_bytes()).expect(
                                        "Client stream write",
                                    );
                                }
                                Err(e) => {
        ...
        info!("Averaged {} packets per second with {} connections per second", packets_sent_total.load(Ordering::SeqCst) / time_used as usize,
